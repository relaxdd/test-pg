...
class Controller {
  public async createUser(req: Request<{ id: string }>, res: Response, next: NextFunction): Promise<Response | void> {
    ...
    try {
      ...
      await db.none(query, [login, email, passwordHash])
  ...
  public async getAllUsers(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
      ...
      const users = await db.any<IUser>(query)
  ...
  public async getOneUser(req: Request<{ id: string }>, res: Response, next: NextFunction): Promise<Response | void> {
    try {
      const query = 'SELECT * FROM public.users WHERE id = $1'
      const user = await db.oneOrNone<IUser>(query, +req.params.id)
  ...
  public async updateUser(req: Request<{ id: string }>, res: Response, next: NextFunction): Promise<Response | void> {
    ...
    try {
      ...
      await db.none(query, [...map.map(it => it.value), userId])
